...
class AudioClassificationModule(LightningNet):
    ...
    Supports multiclass classification with CrossEntropyLoss and standard metrics
    (accuracy, F1). Designed for use with SSL backbones or direct audio features.
    """
    ...
    def model_step(
        ...
        targets = batch.targets[self.hparams.target_name].long().squeeze()
    ...
    def training_step(self, batch: ModelBatch, batch_idx: int):
    ...
    def validation_step(self, batch: ModelBatch, batch_idx: int):
        loss, output, targets = self.model_step(batch)
        preds = output.logits
    ...
    def test_step(self, batch: ModelBatch, batch_idx: int):
        loss, output, targets = self.model_step(batch)
        preds = output.logits
    ...
    def optimizer_step(self, epoch, batch_idx, optimizer, optimizer_closure=None):
        if self.hparams.warmup_rate and self.hparams.warmup_type:
            ...
            for param_group in optimizer.param_groups:
                param_group["lr"] = lr_scale * self.hparams["optimizer"]["lr"]
    ...
    def _get_warmup_lr_scale(self, warmup_steps):
        ...
        elif self.hparams.warmup_type == "cosine":
            import numpy as np

            return 0.5 * (1 + np.cos(np.pi * (1 - float(self.trainer.global_step + 1) / warmup_steps)))
        return 1.0
